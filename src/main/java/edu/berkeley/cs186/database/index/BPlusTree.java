...
public class BPlusTree {
    ...
    public Optional<RecordId> get(DataBox key) {
        ...
        LeafNode result = this.root.get(key);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        Iterator<RecordId> iterator = leafNode.scanGreaterEqual(key);
        if(!iterator.hasNext()){
                throw new NoSuchElementException();

        }else{
            return new BPlusTreeIterator(leafNode,iterator);
        }
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if (splitInfo.isPresent()) {
            // split the root
            DataBox first = splitInfo.get().getFirst();
            Long second = splitInfo.get().getSecond();
            List<DataBox> keys = new ArrayList<>();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        LeafNode nowLeafNode;
        LeafNode lastLeafNode;
        Iterator<RecordId> nowIterator;

        BPlusTreeIterator(BPlusNode root) {
            this.nowLeafNode = root.getLeftmostLeaf();
            this.lastLeafNode = null;
            this.nowIterator = nowLeafNode.scanAll();
        }
        BPlusTreeIterator(LeafNode nowLeafNode,Iterator<RecordId> nowIterator){
            this.nowLeafNode=nowLeafNode;
            this.nowIterator=nowIterator;
            this.lastLeafNode=null;
        }
        @Override
        public boolean hasNext() {
           while(!nowIterator.hasNext()){
               Optional<LeafNode> rightSibling = nowLeafNode.getRightSibling();
               if(!rightSibling.isPresent()){
                   break;
               }else{
                   lastLeafNode=nowLeafNode;
                   nowLeafNode=rightSibling.get();
                   nowIterator=nowLeafNode.scanAll();
               }
           }
          return nowIterator.hasNext();
        }
        ...
        public RecordId next() {
           ...
           if(nowIterator.hasNext()){
               return nowIterator.next();
           }else{
               throw  new NoSuchElementException();
           }
        }
